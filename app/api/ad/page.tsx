...
export default function AdAdminPage() {
  ...
  async function onSave() {
    try {
      ...
      if (!url) return alert('Wybierz plik albo zostaw istniejący obraz.');
  ...
  return (
    <div className="min-h-screen bg-white p-6">
      <div className="max-w-3xl mx-auto">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Ustaw obraz reklamy</CardTitle>
          </CardHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="rounded-xl border overflow-hidden bg-zinc-50">
                  {currentUrl ? (
                    <img src={currentUrl} alt="Reklama" className="w-full h-64 object-cover" />
                  ) : (
              ...
              <div>
                ...
                <Button className="mt-4 w-full md:w-auto" onClick={onSave} disabled={saving}>
                  <Save className="h-4 w-4 mr-2" />
                  {saving ? 'Zapisywanie...' : 'Zapisz reklamę'}
                </Button>
              </div>
            </div>

            <p className="text-xs text-zinc-500">
              Ten obraz pojawi się na każdej karcie samochodu, w prawym boksie.
            </p>
          </CardContent>
        </Card>
      </div>
