...
export default function Gallery({ images, videoUrl }: Props) {
  ...
  const isVideoSlide = (src: string) => isYouTube(src);
  ...
  const photoSources = React.useMemo(() => (images || []).filter(Boolean), [images]);
  ...
  return (
    <div className="space-y-3">
      <div className="relative group">
        ...
        >
          {items.map((src, i) => {
            const isVid = isVideoSlide(src);
            return (
              <div key={i} className="min-w-full snap-center relative">
                {isVid ? (
                  <iframe
                    src={getYouTubeEmbed(src) || undefined}
                ...
                {/* delikatny badge play na slajdzie wideo */}
                {isVid && (
      ...
      {items.length > 1 && (
        <div className="grid grid-cols-7 gap-3">
          {items.slice(0, 10).map((src, i) => {
            ...
            const asVideo = isVideoSlide(src);

            if (asVideo) {
              const id = getYouTubeId(src);
              const thumb = getYouTubeThumb(src);

              return (
                <button
                  key={i}
                  onClick={() => goTo(i)}
                  className={`relative h-16 w-full rounded-lg overflow-hidden cursor-pointer ${
                ...
                >
                  ...
                  <img
                    ...
                    onError={(e) => {
                      // fallback gdy maxresdefault nie istnieje
...
function ImageModal({
  ...
  const [i, setI] = React.useState(Math.max(0, Math.min(sources.length - 1, initialIndex)));
