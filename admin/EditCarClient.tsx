...
/* ---------------- Normalizery wartości ze „starych” zapisów ---------------- */
...
function norm<T extends readonly string[]>(
  ...
  // podmiana aliasów (np. "benzyna" -> "Benzyna")
  const aliasHit = aliases[raw.toLowerCase()];
  if (aliasHit) return aliasHit;

  // jeśli już pasuje do dozwolonych, zostaw
  const asAllowed = allowed.find((x) => x === raw) as T[number] | undefined;
  if (asAllowed) return asAllowed;

  // spróbuj case-insensitive
  const ci = allowed.find((x) => x.toLowerCase() === raw.toLowerCase());
...
type EditCarClientProps = {
  car: any; // dane auta pobrane w komponencie serwerowym i przekazane do klienta
};
...
export default function EditCarClient({ car }: EditCarClientProps) {
  ...
  // znormalizowane wartości startowe (radzą sobie z 'benzyna' itp.)
  const defaults = useMemo<Partial<CarFormData>>(
    () => ({
      title: car?.title ?? '',
      brand: car?.brand ?? '',
      model: car?.model ?? '',
      ...
      engineCapacityCcm: car?.engineCapacityCcm ?? car?.engine_capacity_ccm ?? car?.engine_capacity,
      powerKw: car?.powerKw ?? car?.power_kw,
      ...
      origin: car?.origin ?? car?.country ?? car?.kraj,
      registeredIn: car?.registeredIn ?? car?.registered ?? car?.zarejestrowany,
      saleDocument: car?.saleDocument ?? car?.sale_document,
  ...
  const onSubmit = async (data: CarFormData) => {
    try {
      ...
      const updated = {
        ...
        // media
        main_image_path: imageUrls[0] || car.main_image_path,
        images: imageUrls.length ? imageUrls : car.images ?? [],
        video_url: videoUrl ?? car.video_url,
